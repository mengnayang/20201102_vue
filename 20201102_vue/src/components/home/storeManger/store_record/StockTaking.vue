<template>
    <div class="stock_taking_container">
        ...
        <el-card>
            ...
            <el-table :data="isLazzy2 ? stockingGoodsList_lazzy : stockingGoodsList" border stripe v-show="isSecond">
                <el-table-column label="商品图片" fixed align="center" width="120px">
                    <template slot-scope="scope">
                        <img :src="$store.state.imgBaseURL + scope.row.goodsPicture" alt="商品图片" class="all_img">
                    </template>
                </el-table-column>
        ...
        <!-- 查看详细信息列表 -->
        <el-dialog title="盘点商品信息" :visible.sync="lookDialog" width="800px">
            <el-form>
                <el-row>
                ...
                <el-row>
                    <el-col :span="6" :offset="2">
                        <el-form-item label="商品图片:">
                            <img :src="$store.state.imgBaseURL + current.goods.goodsPicture" alt="商品图片" class="all_img">
                        </el-form-item>
                    </el-col>
                </el-row>
            </el-form>
        ...
        <!-- 修改详细信息列表 -->
        <el-dialog title="盘点商品信息" :visible.sync="editDialog" width="800px">
            <el-form label-width="80px">
                ...
                <el-row>
                    ...
                    <el-col :span="9" :offset="1">
                        <el-form-item label="商品图片:">
                            <img :src="$store.state.imgBaseURL + current.goods.goodsPicture" alt="图片" class="all_img">
                        </el-form-item>
                    </el-col>
                </el-row>
            </el-form>
    ...
    export default{
        ...
        methods:{
            ...
            searchPartStock3() {
                this.isLazzy3 = true
                let goods = {
                    goodsCategoryId:this.selected3.categoryId
                }
                let data = {
                ...
                this.$axios.post('/stocktaking/selectStocktakingGoodsfindByConditions', this.$qs.stringify(data),{
                    headers:{
                        staffToken:window.sessionStorage.getItem('staffToken')
                    }
                })
                .then((res) => {
                    if (res.data.success) {
                        this.goodsStockAList_lazzy = res.data.goodsStockAList
                        this.queryInfo3.total = res.data.recordSum
                        this.categoryList = res.data.categoryList
                        this.goodsStockAList_lazzy.map((item) => {
            ...
            //修改该用户所分配的某商品的详细信息
            editDialog1() {
                let flag = this.onNumChange('盘点数量',this.current.stocktaking.stocktakingNum)
                if (flag == -1) {
                    return
                }
                let stocktaking = {
            ...
            handleSelectionChange(selections) {
                ...
                if (this.tempCategoryList.length == 0) {
                    for (let i = 0; i < this.multipleSelection.length; i++) {
                        this.tempCategoryList.push(this.multipleSelection[i])
                    }
                } else {
                    for (let i = 0; i < this.multipleSelection.length; i++) {
                        let flag = false
                        for (let j = 0; j < this.tempCategoryList.length; j++) {
                            if (this.tempCategoryList[j].stockGoodsId == this.multipleSelection[i].stockGoodsId) {
                                flag = true
                                break
                            }
                        }
